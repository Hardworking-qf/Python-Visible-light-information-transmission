...
def decode_to_hamming(ResultMatrix,FrameSize,canWrite):
    result=[]
    count=0
    tempresult=0
    for i in range(FrameSize):
...
def Hamming_decode(data):
    ...
    for i in data:
        fault=0
        fault+=((i>>11)^(i>>9)^(i>>7)^(i>>5)^(i>>3)^(i>>1))&0b1
        fault+=(((i>>10)^(i>>9)^(i>>6)^(i>>5)^(i>>2)^(i>>1))&0b1)<<1
        fault+=(((i>>8)^(i>>7)^(i>>6)^(i>>5)^i)&0b1)<<2
        fault+=(((i>>4)^(i>>3)^(i>>2)^(i>>1)^i)&0b1)<<3
        if fault!=0:
            data^=1<<(fault-1)
        tempres=0
        tempres+=i&0b1111
        tempres+=(i&0b11100000)>>1
        tempres+=(i&0b1000000000)>>2
...
show_Analyze_Result(cv2.GaussianBlur(AffineResult.copy(),(5,5),3),AffineCellSize, FrameSize)

DecodeResult = Decode(cv2.GaussianBlur(AffineResult.copy(),(5,5),3), AffineCellSize, FrameSize)
...
#plt.imshow(canWrite)
#plt.show()
Hanming=decode_to_hamming(DecodeResult, FrameSize, canWrite)
print(Hamming_decode(Hanming))
